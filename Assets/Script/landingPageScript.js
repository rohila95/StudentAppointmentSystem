...
function start()
{
	$(document).ready(function() {
        ...
        $(document).on("click",".editAppointment",function(){
  	    	...
  	    	//to make any number of appointments to be initiated at once by Faculty -- currently left unimplemented have to revisit this 
	    		...
	    		/*var preSem = jQuery.trim(ParentTR.find(".stuSem .oSpan").text()); 	    		
  	    		...
  	    		if(prePost.indexOf("PHD") > -1){
  	  			    postSelectStr = '<span class="tSpan"> <select class="form-control postIP" name="postIP"><option value="0">select post</option><option value="PHD_GRA">PHD_GRA</option><option value="PHD_SGRA">PHD_SGRA</option><option value="PHD_GTA">PHD_GTA</option><option value="PHD_Grader">PHD_Grader</option></select></span>';
  	    		}
	  			...
	  			if(ParentTR.find(".stuProj .projSel").length == 0){
		  			...
		  			preProjName= preProjName.replace(/\-/g, '/')
		  			preProjVal = preProjName +"-"+preProjId ;
		  			
		  			ajaxGetProjByStaffIDAddExistingAppInEdit(ParentTR, preProjVal);
        ...
        $(document).on("click",".cancelEditAppointment",function(){
  	    	 /*ParentTR = $(this).parents("tr");  	
  	    	inBTWAddition = false;
    		...
    		if(adminType == 1){
    			if(ParentTR.find(".stuProj .projSel").length == 0){
    				...
    				// dont exactly remember why this condition is put
        			ParentTR.find(".stuProj .oSpan").hide();
    			}
        ...
        $(document).on("click",".submitEditChanges",function(){
  	    	...
  	    	var formdata = curRowParent.find("input").serializeArray();
	  	  	...
	  	  	$(formdata).each(function(index, obj){
	  	  	 ...
	  	  	 var projIp = "",projId="", projectConfirmStr = "";
	  	  			...
	  	  			if(projId.split("-").length == 2){
	  	  				projId = projId.split("-")[1];
	  	  			}
